...
def read_grayscale_values_from_txt(txt_path):
    with open(txt_path, 'r') as f:
        ...
        grayscale_values = []
        for line in lines:
            grayscale_values.extend([int(value) for value in line.strip().split(',')])
        #grayscale_values = [x - 1 for x in grayscale_values]  # Adjusting values as needed
        print(f"Считанные значения из файла {txt_path}: {grayscale_values}")
...
def is_clockwise(contour):
    ...
    print(f"Значение value для контура: {value}")
    return value < 0
    ...
    for i, p1 in enumerate(contour1):
        for j, p2 in enumerate(contour2):
            ...
            if distance_min < 0:
                ...
                idx1 = i
                idx2 = j
    print(f"Индексы слияния: idx1 = {idx1}, idx2 = {idx2}, минимальное расстояние: {distance_min}")
    return idx1, idx2
...
def merge_contours(contour1, contour2, idx1, idx2):
    ...
    print(f"Длина итогового контура после слияния: {len(contour)}")
    return contour
...
def extract_contours(image_path, txt_path, buffer_size):
    ...
    for grayscale_value in grayscale_values:
        for i in range(grayscale_value - buffer_size, grayscale_value + buffer_size + 1):
            mask[image == i] = 255  # Создание маски для всех значений серого

    polygons = mask2polygon(mask)

    contours_dict = {int(grayscale_value): polygons for grayscale_value in grayscale_values}

    for obj_id, contours in contours_dict.items():
        print(f"Найдено {len(contours)} контуров для объекта с id {obj_id}")
...
if __name__ == "__main__":
    image_dir = "results"
    txt_dir = "output"
    output_contours_dir = "contours"
    output_yolo_dir = "TXTs"
